...
#for i,j in enumerate(train_json[2000]['MC_ans']):
    #print(i)
    #print(j)
...
def encode_question(img_json, param_dict, wtoi):
    ...
    for i,img in enumerate(img_json):
        ...
        for k,w in enumerate(img['final_question']):
            if k < max_length:
                lable_arrays[i,k] = wtoi(w)
                
    return lable_arrays, label_length, question_id
...
get_unqiue_img(train_json[:10])


# In[44]:


#count = {'a':10,'b':12,'c':7,'d':6,'e':17,'g':14}
#sorted([(count,w) for w,count in count.items()], reverse=True)
#list(count.keys())
...
def main(params):
    ...
    #暂时先取10000张图片看看效果……
    imgs_train = imgs_train[:10000]
    imgs_val = imgs_val[:10000]
    ...
    if params['word2vector_method'] == 1:
        ...
        # 看看还剩了多少#
        print("number of training image left from previous 10000: ",len(imgs_train))
        print("number of training image left from previous 10000: ",len(imgs_val))
        ...
        ques_val, ques_length_val, question_id_test = encode_question(imgs_val, params, wtoi)
        ...
        f.create_dataset("ans_test", dtype='uint32', data=ans_val)
        
        f.create_dataset("ques_id_train", dtype='uint32', data=question_id_train)
        f.create_dataset("ques_id_test", dtype='uint32', data=question_id_val)
        ...
        f.create_dataset("ques_len_val", dtype='uint32', data=ques_length_val)
        f.create_dataset("MC_ans_train", dtype='uint32', data=MC_ans_train)
        f.create_dataset("MC_ans_val", dtype='uint32', data=MC_ans_val)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_h5', default='/datasets/home/home-03/98/898/wel031/san_vqa/data/vqa_data_prepro.h5', help='output h5 file')
  
    # options
    # 这里有个区别，稍微把question的长度控制的更小一些……
    parser.add_argument('--max_length', default=14, type=int, help='max length of a caption, in number of words. captions longer than this get clipped.')
